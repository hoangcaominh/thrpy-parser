...
/**
 * Parses th16 replay and returns the replay's information
 * @param {Buffer} replay 
 * @returns Information about the replay
 */
function parse16(replay) {
    ...
    // Replay
    const R = new Th16(new KaitaiStream(unlzss(compData)))

    // Base info
    const info = {
        game: C.GAME.TH16.ID,
        shot: R.header.shot,
        season: R.header.season,
    ...
    R.stages.forEach((stage) => {
        stages.push({
            ...
            season_power: stage.seasonPower
        })
    })
    info["stages"] = stages
